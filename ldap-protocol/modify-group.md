...
```bash
nxc ldap <ip> -u user -p pass -M modify-group -o USER=TargetUser GROUP=TargetGroup
```
...
```bash
nxc ldap <ip> -u user -p pass -M modify-group -o USER=TargetUser GROUP=TargetGroup REMOVE=True
```
