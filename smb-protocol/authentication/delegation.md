...
RBCD (like traditional constrained delegation) is driven by Microsoft’s **Services for User (S4U)** extensions, not by the core Kerberos exchange alone. **S4U2Self** is a TGS-REQ where `sname` names **your own** principal, **`PA-FOR-USER`** names the user to impersonate, and the KDC returns a service ticket for yourself **encrypted with your account’s long-term key**, carrying that user’s **PAC**. **S4U2Proxy** is a second TGS-REQ for the **real target SPN** (e.g. `cifs/host`): you put the S4U2Self ticket in **`additional-tickets`** as evidence so the KDC can issue a normal service ticket whose **PAC** matches the evidence ticket instead of your TGT’s PAC.
...
> **Note:** The KDC may pick an **AES** session key, the next step needs **RC4-HMAC**. If the ticket is not RC4, try requesting the TGT with **`-H`** instead of a password, then re-check with `describeTicket`.
