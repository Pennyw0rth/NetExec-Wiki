---
description: NetExec installation
---

# Installation

{% content-ref url="installation-on-unix.md" %}
[installation-on-unix.md](installation-on-unix.md)
{% endcontent-ref %}

{% content-ref url="installation-on-windows.md" %}
[installation-on-windows.md](installation-on-windows.md)
{% endcontent-ref %}

{% content-ref url="installation-for-mac.md" %}
[installation-for-mac.md](installation-for-mac.md)
{% endcontent-ref %}

{% content-ref url="using-docker.md" %}
[using-docker.md](using-docker.md)
{% endcontent-ref %}

{% content-ref url="manually-building-the-binary.md" %}
[manually-building-the-binary.md](manually-building-the-binary.md)
{% endcontent-ref %}

{% content-ref url="setting-up-tab-completion.md" %}
[setting-up-tab-completion.md](setting-up-tab-completion.md)
{% endcontent-ref %}