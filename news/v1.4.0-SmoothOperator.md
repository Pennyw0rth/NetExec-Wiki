...
A full implementation of the attack is now available in NetExec, indicates the vulnerability by a new flag in the host banner. The implementation was done by [@NeffIsBack](https://x.com/al3x_n3ff).
...
New LDAP flag `--base-dn` provides the ability to manually specify the baseDN for search requests. It finds the objects, if would not be found by the query. Thanks to [@NeffIsBack](https://x.com/al3x_n3ff).
...
After coerce\_plus Module, With the joint development of [@Adamkadaban](https://github.com/Adamkadaban) and [@lodos](https://github.com/lodos2005), coerce attacks can now be performed over MSSQL `-M mssql_coerce` .
...
Thanks to [@deathflamingo](https://github.com/deathflamingo), MSSQL protocol has _enum\_impersonate_, _enum\_logins_, _enum\_links_, _exec\_on\_link_, _link\_enable\_xp_, _link\_xpcmd_ modules. With this modules the functionality of MSSQL has increased even more.
...
If found MSSQL admin user, linked servers config's can bu find `-M enum_links` with query the login for. Thanks to [@NeffIsBack](https://x.com/al3x_n3ff).
