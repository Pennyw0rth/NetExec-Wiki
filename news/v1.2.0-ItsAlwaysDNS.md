...
A big rework of the powershell functionality inside of NetExec has taken place, which eliminates most Bugs and overall improves its usability and stability. Also obfuscation and amsi bypasses were set to non default, as they often got flagged even by Windows Defender. A neat side effect is, that the `ps32` downgrade now bypasses Windows Defender😄\
